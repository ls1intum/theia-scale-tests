...
test.describe("Landing Page Tests (unauthenticated)", () => {
  ...
  test("LandingPage: Login button should be visible", async ({
    landingPage,
  }) => {
    const loginButton = await landingPage.page.getByRole("button", {
      name: "Login",
    });
    await expect(loginButton).toBeVisible();
  });

  test("LandingPage: Login should redirect to Keycloak", async ({
    landingPage,
  }) => {
  ...
  test("LandingPage: Login via the UI", async ({ landingPage }) => {
    ...
    const logoutButton = await landingPage.page.getByRole("link", {
      name: "logout",
    });
...
test.describe("LandingPage: Landing Page Setup", () => {
  ...
  test("LandingPage: Programming language instances should be visible", async ({
    landingPage,
  }) => {
