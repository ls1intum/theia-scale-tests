...
export class ExercisePage {
  ...
  async checkResultScore(expectedResult: string) {
    const resultScore = this.page
      .locator("#exercise-headers-information")
      .locator("#result-score");
  ...
  async startParticipation() {
    const participationButton = await this.page.locator(
      "#start-exercise-" + this.id,
    );
