...
export class ScorpioView extends TheiaView {
    ...
    async getScorpioView(): Promise<FrameLocator> {
        return await this.page.locator('iframe').contentFrame().locator('#active-frame').contentFrame();    
    }

    async getLoginButton(): Promise<Locator> {
        return await (await this.getScorpioView()).getByRole('button', { name: 'Login' });
    }

    async getQuickInputWidget(): Promise<Locator> {
        return await this.page.locator('.quick-input-widget').getByRole('combobox', { name: 'input' });
    }
}
