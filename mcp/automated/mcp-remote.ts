...
const prompt = fs.readFileSync(path.resolve("mcp/virtualstudent.prompt"), "utf8");
...
class MCPClient {
  ...
  async connectToServer() {
    this.transport = new StdioClientTransport({
      command: "npx",
      args: ["@playwright/mcp@latest", `--storage-state=${storageStatePath}`, "--isolated"],
    });
    ...
    while (response.content.some((c) => c.type === "tool_use")) {
      ...
      response.content.forEach((c) => {
        if (c.type === "text") {
          ...
          console.log(`Step ${step} - LLM wants to use tool "${c.name}" with args:`, c.input);
        }
      });
      ...
      // Execute the tool
      const toolResult = await this.mcp.callTool({ name: toolName, arguments: toolArgs });
